...
public class OAILoaderData extends BaseStepData implements StepDataInterface {
    ...
    //must be included for DataBase Data Loading
    /**
	 * Method to extract a row of Data. The method MUST BE implemented
	 * for Data precatching. The logic must be similar to the method processRow
	 * of the BaseStep interface.
	 * @param smi StepMetaInterface
	 * @param sdi StepDataInterface
	 * @param databaseLoad Boolean.TRUE if the step needs to precatch data to a bundled database
	 * @return Boolean.TRUE if there is more data to extract, otherwise Boolean.FALSE
	 * @throws KettleException
	 */
    public Boolean getData(StepMetaInterface smi, StepDataInterface sdi, Boolean databaseLoad)throws KettleException {
		...
		try{		
			...
			//while (data.listRecords != null) {
				...
				//must be included for DataBase Data Loading
				if(databaseLoad) DatabaseLoader.getConnection();
				...
				if(databaseLoad) DatabaseLoader.closeConnection();
				...
				if (data.resumptionToken == null
					...
					try {
						...
						// TODO Auto-generated catch block
						dataLoader.logBasic("IOException while trying to resume from "
						...
						dataLoader.logBasic("SAXException while trying to resume from "
								+ data.resumptionToken + ", trying again.");
